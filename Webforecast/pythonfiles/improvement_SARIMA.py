...
#Loadind the dataset 
traindf = pd.read_csv('downsample100.csv')
...
for column in traindftrans:
    ...
    try:
        best_model = sm.tsa.statespace.SARIMAX(dftrain.test[:-60], order=(best_paramlist[i][0], best_paramlist[i][1], best_paramlist[i][2]),
                                      seasonal_order=(best_paramlist[i][3], best_paramlist[i][4], best_paramlist[i][5], 12)).fit(disp=-1)
    except ValueError:
                continue
...
#     pred = predict["Predict Value"]
#     true = predict['Real Value']
    valmape = []
